...
-- ===== С ИНТЕГРИРОВАННЫМИ ДОПОЛНЕНИЯМИ =====
-- ========================================
...
local function createWelcomeScreen()
    ...
    blackFrame.Size = UDim2.new(0.5, 0, 0.2, 0)
    blackFrame.Position = UDim2.new(0.25, 0, 0.4, 0)
    blackFrame.BackgroundColor3 = Color3.fromRGB(10, 10, 15)
    blackFrame.BackgroundTransparency = 0.15
    ...
    title.TextSize = 18
    title.Font = Enum.Font.GothamLight
    ...
    subtitle.Text = "нажмите в любую точку для продолжения"
    ...
    snowContainer.Parent = welcomeGui

    local function createWelcomeSnow()
        ...
        local endPos = UDim2.new(snow.Position.X.Scale + math.random(-10, 10) / 100, 0, 1.1, 0)
        local tween = TweenService:Create(snow, TweenInfo.new(6 + math.random(0, 4), Enum.EasingStyle.Linear), {Position = endPos})
        tween:Play()
    ...
    clickButton.Activated:Connect(function()
        ...
        TweenService:Create(blackFrame, TweenInfo.new(0.5, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
...
repeat task.wait() until not welcomeScreen.Parent
